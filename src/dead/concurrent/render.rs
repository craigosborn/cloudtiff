...
impl CloudTiff {
    pub async fn render_image_with_mp_limit_par<R: ReadRangeAsync + Clone>(
        &self,
        source: &mut R,
        max_megapixels: f64,
        ...
        let width = ar * height;
        self.render_image_region_par(source, (0.0, 0.0, 1.0, 1.0), (width as u32, height as u32))
            .await
    }

    pub async fn render_region_lat_lon_deg_par<R: ReadRangeAsync + Clone>(
        &self,
        source: &mut R,
        ...
        self.render_region_par(source, epsg, region, dimensions)
            .await
    }

    pub async fn render_region_par<R: ReadRangeAsync + Clone>(
        &self,
        source: &mut R,
        epsg: u16,
        ...
        self.render_pixel_map_par(source, level, pixel_map, dimensions)
            .await
    }

    pub async fn render_image_region_par<R: ReadRangeAsync + Clone>(
        &self,
        source: &mut R,
        region: (f64, f64, f64, f64),
        dimensions: (u32, u32),
        ...
        let tile_results = level
            .stream_tiles_in_region_par(source, region.clone())
            .await;
    ...
    async fn render_pixel_map_par<R: ReadRangeAsync + Clone>(
        &self,
        source: &mut R,
        level: &Level,
        pixel_map: PixelMap,
        ...
        let tile_map: HashMap<usize, Raster> = level
            .stream_tiles_par(source, tile_indices)
            .await
            .into_iter()
            .filter_map(|r| match r {
                Ok(Ok((tile, index, _bounds))) => Some((index, tile)),
                _ => None,
            })
            .collect();
        debug!(
    ...
    async fn _render_pixel_map_par_old<R: ReadRangeAsync + Clone>(
        &self,
        source: &mut R,
        level: &Level,
        pixel_map: PixelMap,
        ...
        for (tile_index, tile_pixel_map) in pixel_map.iter() {
            ...
            let t_tile = Instant::now();
            if let Ok(tile) = level.get_tile_by_index_par(source, *tile_index).await {
    ...
    pub async fn get_tile_at_lat_lon_par<R: ReadRangeAsync>(
        &self,
        source: &mut R,
        level: usize,
        lat: f64,
        ...
        level.get_tile_at_image_coords_par(source, x, y).await
    }
}
